"""
run_A4_auto_v45.py  --  Colour pipeline (legend-driven masks -> Viterbi path -> step 5)
===================================================
...
Changelog
---------
v45: The detection stage is replaced by a legend-driven colour pipeline.  The
     code either side of it is v44 untouched, and the GUI contract is unchanged
     -- same argv, same output directory, same edit_data.json -- so
     run_gui_v2.py only needs the script name bumped.

     Single file on purpose: the mask / curve / verdict code that used to live
     in color_mask.py, color_curves.py and color_pipeline.py is inlined below
     the CLI block.

     What replaced what
       masks      swatch CORE colour (never a mean: on a small marker the
                  anti-aliased halo outnumbers the core and drags the anchor
                  toward paper, which puts the real core outside its own tube)
                  + a per-swatch adaptive Lab radius, capped by half the
                  distance to the nearest rival.
       markers    template cut from the legend swatch, matched on the soft map,
                  thresholded against the score that same template gets on its
                  own swatch -- a per-curve reference, so no global constant.
                  Curves the map cannot anchor fall back to linear unmixing at
                  grid columns only.
       grid       shared x columns agreed across curves.
       path       ONE Viterbi path per curve over its whole domain, replacing
                  the per-column walk: a greedy tracker loses the curve at the
                  first ambiguous column and never recovers.
       points     grid x path, each verified pixel-by-pixel by colour and
                  graded REAL / OCCLUDED / WEAK.  Evidence outranks statistics:
                  a confirmed marker is never vetoed by a density gate.
       segments   path fragments absorbed into point-connected segments; every
                  segment is checked against the ORIGINAL image with a strong
                  ink test and a loose colour test, because a stroke covered by
                  another curve is still opaque but no longer its own hue.
       step 5     ACTIVE / SUPPRESSED / segments handed to 5_correction_color.
                  No torch is involved: CLASS_NAMES is a plain constant and the
                  detector is bypassed by supplying init_points.

     Flags
       --no-color-pipeline   fall back to the v44 walk detector
       --step5 DIR           directory holding 5_correction_color.py
       --step5-iters N       max_iters for the corrector (default 1)

     Known gaps, left deliberately
       * marker SHAPE is not classified from the swatch yet, so step 5 renders
         with CLASS_NAMES[0]; set cd['marker_class'] to fix the render.
       * 5_correction_color.build_iter_figure crashes when l* is None; it is
         patched to a no-op here because it is purely diagnostic.
       * the 21-plot regression set has not been run against this version.
...
_ap.add_argument('--no-color-pipeline', action='store_true',
                 help='use the v44 walk detector instead of the v45 colour pipeline')
_ap.add_argument('--step5', default=None,
                 help='directory holding 5_correction_color.py; enables SSIM correction')
_ap.add_argument('--step5-iters', type=int, default=1,
                 help='max_iters for step 5 (default 1)')
_ap.add_argument('--outlier-filter', action='store_true',
                 help="run v44's post-detection outlier / fake-segment filters "
                      "(off by default, matching unified_server.py)")
...
_V45_DIR = os.path.dirname(os.path.abspath(__file__))
USE_V45 = not _args.no_color_pipeline
STEP5_DIR = _args.step5
STEP5_ITERS = int(_args.step5_iters)


# ============================================================================
# v45 colour engine  (was color_mask.py / color_curves.py / color_pipeline.py)
# ----------------------------------------------------------------------------
# Inlined so there is a single file to version.  Nothing below depends on the
# v44 code above it; the only entry point the pipeline uses is the block near
# the detection seam further down.
#
# Two products of `colour_masks` serve two different jobs and must not be
# swapped: `masks[i]` (exclusive, nearest-swatch) answers WHOSE pixel this is,
# `soft[i]` (non-exclusive) answers WHERE the stroke runs.  Overlapping strokes
# genuinely share pixels, so forcing exclusivity on the trajectory side deletes
# any curve that is drawn underneath another.
# ============================================================================
from scipy.optimize import nnls as _nnls        # linear unmixing only

# ------------------------------------------------- chart furniture / stems --
def _load_segment_detector():
    path = os.path.join(_V45_DIR, '3_segment_detection_v2.py')
...
# ------------------------------------- chart furniture, detected part by part --
"""Chart furniture, detected as four separate things.

The single "long thin dark band" rule that was in use lumps the y-axis, the
x-axis, a reference rule and stray text into one test, and then deletes
whatever trips it.  On one plot that swallowed a data curve's jump line: three
columns at x=140-142 qualified because several curves crossed there and pushed
the dark count over 50% of the plot height, and the vertical guard could not
object because a 3px-wide band's row extent is 3px by construction.

Each part has conditions of its own, so each can be reported and checked.
"""
...
def furniture_parts(img_bgr, plot_area, achro_max=30, dark_max=170,
    ...
    # must hold is that the row is grey, thin, and not at either edge.
    rows = []
...
# ------------------------------- two-pass segments, occlusion-bridged ---------
"""Two-pass segments: B&W first, then per colour with occlusion bridging.

Pass 1 reads the plot as black-and-white and assigns each segment to a curve.
That works where curves are apart, and fails exactly where they are not: the
detector calls a pixel part of a line only when its longest run is at least
twice its perpendicular run, and in a dosing spike four curves, their markers
and their error bars sit inside 30px, so every direction is inked.  Measured on
one plot: 80-86% of ink pixels pass the linearity test in the sparse lower half,
but only 14-28% in the dense upper band at day 0, 4 and 8.

Pass 2 therefore looks at one colour at a time, and splits the ORIGINAL image
three ways -- this colour, some other colour, paper.  Two pieces of the same
curve that are interrupted by ANOTHER CURVE are still one stroke: the ink did
not stop, it went behind something.  A gap over paper is a real break and is
left alone.  Bridges are cut at the grid columns like any other segment, and
anything pass 2 finds that pass 1 already had is dropped as a duplicate.
"""
...
# ------------------------------- vertical strokes and per-curve jumps ---------
"""Vertical strokes, and what each curve does across one.

A dose in a PK plot draws a vertical: the curve arrives at one value and leaves
at another at the same x.  An error bar is also vertical and the same colour,
and where several curves are dosed at once their verticals overlap into a
single blob of ink, so the two cannot be told apart by looking at the vertical
alone.

What separates them is where the curve ATTACHES.  Across a jump the curve
enters on the left at the FOOT and leaves on the right from the TOP; an error
bar hangs off a single point, so whatever enters and leaves does so at the same
height.  The direction is not symmetric -- a PK curve does not step downward at
a dose -- so requiring entry-at-foot and exit-at-top removes the ordinary steep
descents that a magnitude test keeps: measured on one plot, 12 candidate jumps
fell to 5, and those 5 are exactly the dosing events.
"""

def vertical_strokes(ink, min_h=20, L=12):
    """Near-vertical ink, found without reference to colour.

    Opening with a tall thin element keeps only pixels with L contiguous
    vertical neighbours, which erases the gentle parts of every curve and
    leaves the verticals.  Width is NOT used to filter: a stroke swells
    wherever a marker sits on it, and rejecting on width throws away the
    columns where several curves jump together -- the very ones that matter.
    """
    ...
    for k in range(1, n):
        ...
        cx = [np.mean(np.nonzero(sel[r])[0]) + x for r in range(h) if sel[r].any()]
        out.append(dict(xc=float(np.median(cx)), y0=float(y), y1=float(y + h - 1),
                        x0=int(x), x1=int(x + w - 1), sel=sel, oy=int(y)))
...
# ---------------------------------------------------------------- masks ----
"""Generalised legend-driven colour mask extraction.

Stages
  1. locate the graphical swatch of every legend row (no OCR, no text)
  2. estimate each swatch's INK colour from its own core pixels
  3. model anti-aliasing as a tube from ink colour to the paper colour
  4. resolve competition by nearest-swatch assignment
  5. restrict to the plot area, blank the legend rectangle
  6. report chart furniture separately instead of deleting it

The tube radius is NOT a constant: it is estimated per swatch from that
swatch's own pixel spread and then capped by half the distance to its nearest
rival, so two close hues can never claim each other's ink.
"""
...
def colour_masks(img_bgr, plot_area, legend_box, swatch_boxes=None,
                 blend_max=0.80, furniture=None, verbose=False,
                 spatial=False, spatial_win=15, spatial_margin=0.25,
                 spatial_dom=2.0, spatial_iters=2,
                 metric='angle', mag_min=12.0):
    # NOTE: `spatial` defaults OFF.  Deciding pixel ownership destructively at
    # mask time costs recall on curves that are drawn underneath another, and
    # the same question is answered later with far more evidence by the
    # per-pixel verification of an actual candidate point.  Kept available for
    # plots where the masks themselves must be exclusive.
    ...
    for b in swatch_boxes:
        ...
        inks.append(rgb); cores.append(core); achro.append(ac); boxes.append(b)
        alls.append(f[f.min(1) < 244])

    # One radius serves both the exclusive mask and the soft map.  Splitting
    # them (tight mask / wide soft) was tried and reverted: the tight mask stops
    # constraining `mask_support` and the own-ink gate while the wide soft map
    # keeps firing, and detections ran to 77 points with 30 WEAK (color06 alone
    # went 21/9).  The two consumers need to agree on what counts as this ink.
    tols, wide, atols = [], [], []
    for i, rgb in enumerate(inks):
        ...
        wide.append(tols[-1])       # decoupling tried and reverted, see below
        atols.append(adaptive_angle_tolerance(rgb, alls[i], rivals,
                                              achromatic=achro[i]))
    ...
    for i in range(len(inks)):
        if achro[i]:
            ang[i] = px_chroma                 # grey: distance off neutral axis
    ...
    scope[ly0:ly1 + 1, lx0:lx1 + 1] = False                 # stage 5

    # stage 4: a pixel goes to the swatch it fits best, and only if it fits
    if metric == 'angle':
        fit = ang / np.asarray(atols, np.float32)[:, None, None]
        fit[:, chroma < mag_min] = 9e9          # too close to paper to judge
    else:
        fit = perp / np.asarray(tols, np.float32)[:, None, None]
    best = np.argmin(fit, 0)
    ok = (fit.min(0) <= 1.0) & (np.take_along_axis(
        tt, best[None], 0)[0] <= blend_max) & scope

    if spatial:
        best, ok = spatial_refine(best, ok, fit, win=spatial_win,
                                  margin=spatial_margin, dom=spatial_dom,
                                  iters=spatial_iters)
    ...
    for i, b in enumerate(boxes):
        m = ok & (best == i)
        masks[i] = m
        info.append(dict(idx=i, box=b, rgb=inks[i], tol=tols[i],
                         wide=wide[i], atol=atols[i],
                         achromatic=achro[i], px=int(m.sum()),
                         on_furniture=int((m & furniture).sum())
                         if furniture is not None else 0))
    # soft membership, same colour model, NOT exclusive.  Overlapping strokes
    # are physically shared, so trajectory work needs this while ownership
    # decisions need `masks`.
    # the soft map deliberately KEEPS the legend rectangle: a template scored
    # on its own swatch is the per-curve reference the matcher calibrates
    # against.  Callers apply their own scope for detection.
    soft_scope = scope.copy()
    soft_scope[ly0:ly1 + 1, lx0:lx1 + 1] = True
    soft = {}
    for i in range(len(inks)):
        if metric == 'angle':
            m = (np.clip(1.0 - ang[i] / (1.8 * atols[i]), 0, 1)
                 * np.clip((chroma - mag_min) / 10.0, 0, 1))
        else:
            m = (np.clip(1.0 - perp[i] / (2.2 * wide[i]), 0, 1)
                 * np.clip((blend_max - tt[i]) / 0.25, 0, 1))
        soft[i] = (m * soft_scope).astype(np.float32)

    if verbose:
        for d in info:
            print('  #%d box=%s rgb=%s tol=%.1f %s px=%d furniture=%d'
                  % (d['idx'], d['box'], d['rgb'], d['tol'],
                     'ACHRO' if d['achromatic'] else '     ', d['px'],
                     d['on_furniture']))
...
# --------------------------------------------------------------- curves ----
"""Colour-mask -> markers -> shared x-grid -> continuous path -> segments
-> grid-based refinement.

Design note.  Two products of `color_mask.colour_masks` are used for two
different jobs and must not be swapped:

  masks[i]  exclusive, nearest-swatch  -> OWNERSHIP  (whose ink is this?)
  soft[i]   non-exclusive membership   -> TRAJECTORY (where does the stroke go?)

Overlapping strokes genuinely share pixels, so forcing exclusivity on the
trajectory side destroys any curve that is drawn underneath another one.
"""
...
try:
    ...
    _feat = {
        ...
        'ocr-anchored-legend':       "_pcm_detect_legend_via_ocr" in _self_src,
        'spike-recovery-optin':      "SPIKE_RECOVERY" in _self_src,
...
# Skip achromatic augmentation when the OCR-anchored fallback supplied the palette:
# it already enumerated every legend entry (one per label line), so there is no
# missing black/grey curve to recover, and a muted near-grey chromatic swatch (e.g.
# a desaturated blue) must not be re-added as a phantom 'grey' bucket that splits
# one curve into two.
    ...
    if _grid_now and _grid_now.get('cells') and _lb is not None:
        ...
        if _colimg.size > 0:
            ...
            for _q in range(1, _dn):
                ...
                # Reject dark blobs hugging the box edge: a user-drawn box that
                # starts at the legend border/title picks up that dark rule or the
                # top of the label text as a phantom "swatch row". Real swatch rows
                # sit a few px inside the box, so require a small margin.
                if _qy < _lby0 + 6 or _qy > _lby1 - 6:
                    continue
...
# ============================================================================
# v45: replace the walk detections with the colour pipeline
# ----------------------------------------------------------------------------
# `all_detections` / `all_results` are the only things the rest of this file
# reads, so swapping them here leaves every downstream writer -- Excel, the
# overlays, colormasks.json, edit_data.json -- exactly as it was in v44.
# ============================================================================
_V45_STEP5 = None
if USE_V45:
    try:
        ...
        # Swatch boxes come from v44's UNIFIED legend grid, not from a fresh
        # scan.  The row-banding scan in find_legend_swatches only splits on a
        # fully blank row, so a legend with ~15px row pitch merges into ONE
        # band: measured 1 swatch instead of 4-6 on four of five test plots.
        # v44 already solved this -- it snaps swatches to verified column/row
        # intersections and reports the residual -- and using its grid also
        # makes the pairing below exact instead of a colour guess.
        # Swatch boxes: prefer v44's legend grid over a fresh scan.  The
        # row-banding scan in find_legend_swatches only splits on a fully blank
        # row, so a legend with ~15px row pitch merges into ONE band -- it
        # measured 1 swatch instead of 4-6 on four of five test plots.  v44
        # already snaps swatches to verified column/row intersections, and
        # using its grid also makes the pairing below exact instead of a guess.
        # Whichever source yields one box per curve wins.
        _real0 = [c for c in COLORS
                  if not str(c.get('name', '')).endswith('_sink')]

        def _boxes_from(centres, lbl):
            if not centres:
                return None, lbl
            _ry = sorted({int(round(c[1])) for c in centres})
            _p = (float(np.median(np.diff(_ry))) if len(_ry) > 1
                  else max(10.0, (_lb[3] - _lb[1]) / 2.0))
            _hh = max(4, int(round(0.45 * _p)))
            _hw = max(8, int(round(0.90 * _p)))
            return ([(int(round(c[0])) - _hw, int(round(c[1])) - _hh,
                      int(round(c[0])) + _hw, int(round(c[1])) + _hh)
                     for c in sorted(centres, key=lambda q: (q[0], q[1]))],
                    f"{lbl} (row pitch {_p:.0f}px)")

        _cands = []
        _ug = globals().get('_LAST_UNIFIED_GRID') or {}
        if _ug.get('cols') and _ug.get('rows') and _ug.get('cells'):
            _cc = []
            for _k in sorted(_ug['cells'],
                             key=lambda k: (int(k.split(',')[0]),
                                            int(k.split(',')[1]))):
                _ci, _ri = (int(q) for q in _k.split(','))
                _cc.append((_ug['cols'][_ci], _ug['rows'][_ri]))
            _cands.append(_boxes_from(_cc, "v44 unified legend grid"))
        _rg = globals().get('_LAST_LEGEND_GRID') or {}
        if _rg.get('cells'):
            _cands.append(_boxes_from([(c[0], c[1]) for c in _rg['cells']],
                                      "v44 raw legend table"))
        try:
            _scan = find_legend_swatches(img, _lb)
            _cands.append(_boxes_from([((b[0] + b[2]) / 2.0, (b[1] + b[3]) / 2.0)
                                       for b in _scan], "own swatch scan"))
        except Exception:
            pass
        ...
        if _cands:
            ...
            _sw_boxes, _sw_src = (_exact[0] if _exact
                                  else max(_cands, key=lambda q: len(q[0])))
            print(f"  [v45] {len(_sw_boxes)} swatch box(es) from {_sw_src}"
                  + ("" if len(_sw_boxes) == len(_real0)
                     else f"  -- WARNING {len(_real0)} curve(s) expected"))

        # Furniture, detected as four separate things instead of one "long thin
        # dark band" rule.  The lumped rule deleted a data curve's jump line:
        # three columns qualified because several curves crossed there and
        # pushed the dark count over 50% of the plot height, and the vertical
        # guard could not object because a 3px band's row extent is 3px by
        # construction.  Each part now has its own conditions, and an axis must
        # be mostly GREY -- which is what tells it from a coloured stroke.
        _fur, _fparts, _frep = furniture_mask(img, _pa)
        print('  [furniture] y-axis %s | x-axis %s | rule %s | text %d blob(s)'
              % (_frep.get('y_axis'), _frep.get('x_axis'),
                 _frep.get('rule'), _frep.get('text', 0)))
        _masks, _soft, _info = colour_masks(img, _pa, _lb, metric='tube',
                                            spatial=False, furniture=_fur,
                                            swatch_boxes=_sw_boxes)
        # Chart furniture must leave the masks AND the soft maps, not merely be
        # reported.  An axis or a reference rule is dead straight, spans the
        # whole width and costs nothing in |dy|, so for an achromatic swatch it
        # is the globally cheapest Viterbi path: with a user plot area that
        # includes the x-axis, black's path sat on y=643 for all 673 columns
        # and the curve collapsed to 4 points.  chart_furniture_mask already
        # refuses to touch a genuinely flat data curve (median column extent
        # >= 6px), so removing what it does flag is safe.
        # Keep an unfiltered copy for MARKER detection.  Furniture has to go
        # from the path and the strokes -- an axis is straight, full width and
        # free in |dy|, so a path will lie on it all the way across.  But a
        # marker at time zero is DRAWN ON the y-axis, and deleting the axis
        # deletes the marker with it: on one plot every curve had ink at the
        # axis column and not one of them produced a point there.
        # A chromatic marker is unbothered by achromatic ink, so for those
        # curves detection reads the unfiltered map; an achromatic curve would
        # swallow the axis, so it keeps the filtered one.
        _soft_raw = {_k: _soft[_k].copy() for _k in _soft}
        _chromatic = {}
        for _k in range(len(_info)):
            _c3 = [int(v) for v in _info[_k]['rgb']]
            _chromatic[_k] = (max(_c3) - min(_c3)) > 25
        if _fur.any():
            _nf = sum(int((_masks[_k] & _fur).sum()) for _k in _masks)
            for _k in list(_masks):
                _masks[_k] = _masks[_k] & ~_fur
                _soft[_k] = _soft[_k] * (~_fur)
                if not _chromatic.get(_k, True):
                    _soft_raw[_k] = _soft[_k]      # achromatic: filtered only
            print(f"  [v45] furniture removed from masks/soft maps: {_nf} px "
                  f"({sum(_chromatic.values())}/{len(_info)} chromatic curves "
                  f"keep it for marker detection)")

        # Match the pipeline's swatches to this run's COLORS.
        #
        # BY POSITION, not by colour.  Both lists are read from the same legend
        # top-to-bottom, so index i is index i.  Nearest-colour matching was
        # tried first and silently SWAPPED two curves: v44's swatch_rgb for a
        # small open marker sits in the anti-aliased halo, tens of Lab units
        # from the core this pipeline reads, so the greedy nearest pick can
        # land on a neighbour and the log still reports a full 9/9 match.
        # Colour is kept only as a cross-check that prints a warning.
        # the black SINK is not a data curve and has no legend swatch
        _real = [c for c in COLORS
                 if not str(c.get('name', '')).endswith('_sink')]
        _pairs = {}
        if len(_info) == len(_real):
            for _q, _cd in enumerate(_real):
                _pairs[_cd['name']] = _q
            _bad = 0
            for _cd in _real:
                _j = _pairs[_cd['name']]
                _ref = _to_lab(tuple(int(v) for v in
                                     (_cd.get('swatch_rgb') or _cd['mean_rgb'])))
                _dself = float(np.linalg.norm(_to_lab(_info[_j]['rgb']) - _ref))
                _dbest = min(float(np.linalg.norm(_to_lab(_d2['rgb']) - _ref))
                             for _d2 in _info)
                if _dself > _dbest + 12.0:
                    _bad += 1
                    print(f"  [v45] WARNING {_cd['name']} -> slot {_j}: colour "
                          f"distance {_dself:.0f} but a closer slot exists "
                          f"({_dbest:.0f}); legend order and colour disagree")
            print(f"  [v45] {len(_info)} swatches matched to {len(_real)} "
                  f"curve(s) by legend order ({_bad} colour warning(s))")
        else:
            # counts differ -- fall back to greedy colour matching
            _pal = [_to_lab(d['rgb']) for d in _info]
            _taken = set()
            for _cd in _real:
                _ref = _to_lab(tuple(int(v) for v in
                                     (_cd.get('swatch_rgb') or _cd['mean_rgb'])))
                _best, _bd = None, 1e9
                for _j, _p in enumerate(_pal):
                    if _j in _taken:
                        continue
                    _d = float(np.linalg.norm(_p - _ref))
                    if _d < _bd:
                        _bd, _best = _d, _j
                if _best is not None:
                    _pairs[_cd['name']] = _best
                    _taken.add(_best)
            print(f"  [v45] swatch count {len(_info)} != {len(_real)} curve(s) "
                  f"-- matched {len(_pairs)} by colour (order unusable)")

        _sc = Scorer(img, _info, _fur)
        _scope = np.zeros(img.shape[:2], bool)
        _scope[_pa[1]:_pa[3] + 1, _pa[0]:_pa[2] + 1] = True
        if LEGEND_BOX:
            _scope[_lb[1]:_lb[3] + 1, _lb[0]:_lb[2] + 1] = False

        _tmpl, _mk, _ref_sc, _smap = {}, {}, {}, {}
        for _i, _d in enumerate(_info):
            _tmpl[_i] = marker_template(img, _d['box'], _d['rgb'],
                                        [_q['rgb'] for _q in _info if _q is not _d],
                                        _d['tol'])
            _p, _r = detect_markers(_soft_raw[_i], _tmpl[_i], _d['box'], _scope,
                                    alpha=MARKER_ALPHA)
            _mk[_i], _ref_sc[_i] = _p, _r
            _smap[_i] = (None if _tmpl[_i] is None else
                         cv2.matchTemplate(_soft[_i],
                                           _tmpl[_i].astype(np.float32),
                                           cv2.TM_CCORR) / max(_tmpl[_i].sum(), 1))

        _endm = [list(_d['rgb']) for _d in _info] + [[255, 255, 255]]
        # minimum column separation = the legend marker's own width
        _mw = [t.shape[1] for t in _tmpl.values() if t is not None]
        _minsep = 0.8 * float(np.median(_mw)) if _mw else 0.0
        _grid, _, _ = build_grid({str(k): v for k, v in _mk.items()},
                                 min_sep=_minsep)
        for _i in _mk:
            if _tmpl[_i] is None or len(_mk[_i]) >= UNMIX_MIN:
                continue
            _sup = mask_support(_masks[_i])
            if _sup is None:
                continue
            _got = unmix_candidates(img, _endm, _i, _tmpl[_i], _grid, _sup,
                                    _pa, furniture=_fur, thr=0.55)
            _new = [q for q in _got
                    if not any(abs(q[0] - m[0]) <= 6 for m in _mk[_i])]
            if _new:
                _mk[_i] = sorted(_mk[_i] + _new, key=lambda q: q[0])
                print(f"  [v45] unmix recovered {len(_new)} marker(s) "
                      f"for slot {_i}")
        _grid, _, _ = build_grid({str(k): v for k, v in _mk.items()},
                                 min_sep=_minsep)
        print(f"  [v45] grid {len(_grid)} columns: {[round(g) for g in _grid]}")

        _new_det, _new_res, _s5 = {}, {}, []
        for _cd in COLORS:
            _cn = _cd['name']
            _i = _pairs.get(_cn)
            if _i is None or _tmpl[_i] is None:
                continue
            _sup = mask_support(_masks[_i])
            if _sup is None:
                continue
            _lo, _hi = _sup
            _path, _val = viterbi_path(_soft[_i], _lo, _hi, _pa[1], _pa[3])
            _prof = fragment_profile(_val >= 0.18)
            _keep = fragment_density_filter(_val >= 0.18, _prof)
            _filled, _ = valid_domain(
                np.where(_keep, _val, 0.0),
                anchors=[int(round(q[0])) - _lo for q in _mk[_i]],
                own_cols=_masks[_i][:, _lo:_hi + 1].any(0),
                gap_max=_prof['gap_budget'], dens=0.45)
            _vf = (lambda gx, gy, _ii=_i: _sc.patch(_ii, _tmpl[_ii], gx, gy))
            _P = refine_on_grid(_path, _filled, _grid, _mk[_i], _lo,
                                own=_masks[_i], verify=_vf,
                                verify_own=VERIFY_OWN, verify_paper=VERIFY_PAPER)
            _fin = []
            for _x, _y, _src in _P:
                if _src == 'refined':
                    _y, _ = snap_y(_smap[_i], _tmpl[_i], _ref_sc[_i], _x, _y)
                _o, _ot, _paf = _sc.patch(_i, _tmpl[_i], _x, _y)
                _v = verdict(_o, _ot, _paf)
                if _v == 'PHANTOM' or (1 - _paf) < INK_MIN:
                    continue
                _fin.append((_x, _y, _src, _v))

            _segA = path_to_segments(_path, _filled, _grid)
            try:
                _segA = _segments_off_stems(_segA, confirmed_stems(_masks[_i]),
                                            frac=0.5)
            except Exception:
                pass
            _segB = [(_fin[k][0], _fin[k][1], _fin[k + 1][0], _fin[k + 1][1])
                     for k in range(len(_fin) - 1)]
            _used = [False] * len(_segA); _merged = []
            for _b in _segB:
                _ink, _colf = _sc.line(_i, _b)
                if _ink < SEG_INK or _colf < SEG_COLOUR:
                    continue           # chord across blank paper; keep the parts
                for _k2, _a in enumerate(_segA):
                    if absorbs(_a, _b):
                        _used[_k2] = True
                _merged.append(_b)
            _segs = cut_at_grid(
                _merged + [_a for _k2, _a in enumerate(_segA) if not _used[_k2]],
                _grid, min_len=5.0)

            # Every segment endpoint must carry a data point.  Where the path
            # ends -- and especially where a marker sits ON an axis, whose ink
            # the furniture mask just removed -- no candidate ever reached the
            # grid.  Verify those endpoints directly against the ORIGINAL image
            # with the legend swatch template: Scorer reads raw Lab, not the
            # furniture-stripped mask, so an axis-covered marker still scores.
            # ONLY the curve's two extremities.  Interior segment ends are
            # Douglas-Peucker curvature breaks and grid cuts, not measurements;
            # recovering at those added 10 spurious points on one curve.  The
            # extremities are the case that needs help: a marker sitting on an
            # axis loses its ink to the furniture mask and never reaches the
            # grid, so it is checked against the ORIGINAL image instead.
            _ends_all = set()
            for _sx0, _sy0, _sx1, _sy1 in _segs:
                _ends_all.add((round(_sx0, 1), round(_sy0, 1)))
                _ends_all.add((round(_sx1, 1), round(_sy1, 1)))
            _ends = set()
            if _ends_all:                      # the curve's two extremities only
                _xs_e = sorted(_ends_all, key=lambda q: q[0])
                _ends = {_xs_e[0], _xs_e[-1]}
            _added_e = 0
            for _ex, _ey in sorted(_ends):
                if any((_ex - _qx) ** 2 + (_ey - _qy) ** 2 <= 8 * 8
                       for _qx, _qy, _s, _v in _fin):
                    continue
                # Colour alone is not enough here: on a continuous stroke the
                # template footprint is full of this curve's ink whether or not
                # a MARKER is there, so a colour-only test fired at every
                # fragment boundary (+14 spurious on one curve).  Require the
                # template SHAPE to respond too, at a lower bar than detection.
                _best = None
                for _dy in range(-4, 5):
                    for _dx in range(-2, 3):
                        _o2, _ot2, _pa2 = _sc.patch(_i, _tmpl[_i],
                                                    _ex + _dx, _ey + _dy)
                        _sh = 0.0
                        if _smap[_i] is not None and _ref_sc[_i] > 0:
                            _rr2 = int(round(_ey + _dy - _tmpl[_i].shape[0] / 2.))
                            _cc2 = int(round(_ex + _dx - _tmpl[_i].shape[1] / 2.))
                            if (0 <= _rr2 < _smap[_i].shape[0]
                                    and 0 <= _cc2 < _smap[_i].shape[1]):
                                _sh = float(_smap[_i][_rr2, _cc2]) / _ref_sc[_i]
                        if _best is None or (_sh, _o2) > (_best[0], _best[1]):
                            _best = (_sh, _o2, _ot2, _pa2, _dx, _dy)
                _sh, _o2, _ot2, _pa2, _dx, _dy = _best
                if _sh < 0.60 or _o2 < 0.45 or _pa2 > 0.35:
                    continue
                _fin.append((_ex + _dx, _ey + _dy, 'endpoint',
                             verdict(_o2, _ot2, _pa2)))
                _added_e += 1
            if _added_e:
                _fin.sort(key=lambda q: q[0])
                print(f"  [v45] {_cn}: +{_added_e} point(s) recovered at "
                      f"segment endpoints")

            _new_det[_cn] = [{'x': int(round(_x)), 'y': int(round(_y)),
                              'fitness': 1.0, 'px': 1,
                              'source': 'v45-' + _src, 'verdict': _v}
                             for _x, _y, _src, _v in _fin]
            _new_res[_cn] = ([(int(p[0]), int(p[1])) for p in _path], 1.0)
            _cd['_raw_mask_canvas'] = _masks[_i].astype(np.uint8) * 255
            _cd['_clean_mask'] = _masks[_i]

            _act = [{'cx': float(_x), 'cy': float(_y), 'class_name': _cn,
                     'class_idx': _i, 'confidence': 1.0}
                    for _x, _y, _s, _v in _fin]
            _sup_pool = []
            for _g in [q for q in _grid if _lo <= q <= _hi]:
                _j = int(round(_g)) - _lo
                if not (0 <= _j < len(_path)):
                    continue
                _w = [_path[k][1] for k in range(max(_j - 3, 0),
                                                 min(_j + 4, len(_path)))]
                _cy = float(np.median(_w))
                if any((_g - a['cx']) ** 2 + (_cy - a['cy']) ** 2
                       <= PT_MERGE_TOL ** 2 for a in _act):
                    continue
                _sup_pool.append({'cx': float(_g), 'cy': _cy,
                                  'class_name': 'suppressed', 'class_idx': -1})
            _s5.append({'name': _cn, 'class_idx': _i,
                        'marker_class': _cd.get('marker_class'),
                        'init_points': _act, 'init_suppressed': _sup_pool,
                        'segments_override': [[round(float(v), 2) for v in q]
                                              for q in _segs]})
            print(f"  {_cn}: {len(_fin)} data_points  "
                  f"({sum(1 for q in _fin if q[3] == 'REAL')} REAL, "
                  f"{sum(1 for q in _fin if q[3] == 'OCCLUDED')} OCCLUDED, "
                  f"{sum(1 for q in _fin if q[3] == 'WEAK')} WEAK)  "
                  f"| {len(_sup_pool)} suppressed | {len(_segs)} segments "
                  f"| {_prof['kind']}")

        # ---- vertical strokes: the points a column-wise path cannot hold ----
        # A dose draws a vertical, so the curve has TWO values at one x and a
        # path holding one y per column can only ever keep one of them.  The
        # pair is read from the segments either side: entry at the foot, exit
        # from the top.  Additive -- nothing already found is touched.
        try:
            _mh = [(_tmpl[q].shape[0] if _tmpl.get(q) is not None else 8)
                   for q in range(len(_info))]
            # ink = anything that is not paper, NOT the union of the colour
            # masks: a stroke is held together by its anti-aliased edge and by
            # the grey of an error bar cap, none of which any mask owns, and
            # dropping them broke the strokes apart (19 found -> 12).
            _lab_i = _lab_image(img)
            _pap_i = _to_lab((255, 255, 255)).astype(np.float32)
            _bwink = (np.linalg.norm(_lab_i - _pap_i, axis=-1) >= 14) & _scope & ~_fur
            _grid_cols = [float(g) for g in _grid]
            _p1 = [tuple(float(v) for v in _q[:4]) for _q in
                   extract_segments(np.where(_bwink, 0, 255).astype(np.uint8))]
            _p1 = [(_q[0], _q[1], _q[2], _q[3]) if _q[0] <= _q[2]
                   else (_q[2], _q[3], _q[0], _q[1]) for _q in _p1]
            _p1 = cut_at(_p1, _grid_cols)
            _segs_by = {}
            for _q in range(len(_info)):
                _own = _masks[_q]
                _oth = np.zeros_like(_own)
                for _r2 in range(len(_info)):
                    if _r2 != _q:
                        _oth |= _masks[_r2]
                _oth |= (_bwink & ~_own & ~_oth)
                _A = drop_unsupported(
                    _segments_near_mask(_p1, _own.astype(np.uint8), tol=3, frac=0.75),
                    _bwink, _own)
                _rs2 = residual_ink(_own, _A, width=2, min_blob=6)
                _B = [tuple(float(v) for v in _q2[:4]) for _q2 in
                      extract_segments(np.where(_rs2, 0, 255).astype(np.uint8))]
                _B = [(_q2[0], _q2[1], _q2[2], _q2[3]) if _q2[0] <= _q2[2]
                      else (_q2[2], _q2[3], _q2[0], _q2[1]) for _q2 in _B]
                _B = bridge_segments(_B, three_way(_own, _oth))
                _B = cut_at(_B, _grid_cols)
                _B = drop_unsupported(_B, _bwink, _own)
                _B = [_q2 for _q2 in _B if not is_duplicate(_q2, _A)]
                _A2, _B2, _ = resolve_conflicts(_A, _B, _own)
                _segs_by[_q] = _A2 + _B2
            _strokes, _vres = analyse(
                _bwink, [_masks[_q] for _q in range(len(_info))], _segs_by, _mh)
            _nj = 0; _njall = 0; _ndup = 0
            for _cd in COLORS:
                _cn = _cd['name']
                _i = _pairs.get(_cn)
                if _i is None or _cn not in _new_det:
                    continue
                for _r3 in _vres.get(_i, []):
                    if _r3['kind'] != 'jump':
                        continue
                    # A jump fixes BOTH values in this column, so a single
                    # earlier point sitting between them is the same
                    # measurement read once -- keeping it leaves a third
                    # vertex and the reconstruction zigzags through it.
                    _lo3 = min(_r3['y_lo'], _r3['y_hi'])
                    _hi3 = max(_r3['y_lo'], _r3['y_hi'])
                    _new_det[_cn] = [
                        _d4 for _d4 in _new_det[_cn]
                        if _d4.get('source') == 'v45-jump'
                        or not (abs(_d4['x'] - _r3['x']) <= 2.0
                                and _lo3 + 2 < _d4['y'] < _hi3 - 2)]
                    _njall += len(_r3['points'])
                    for _px, _py in _r3['points']:
                        if any((_px - _d3['x']) ** 2 + (_py - _d3['y']) ** 2
                               <= PT_MERGE_TOL ** 2 for _d3 in _new_det[_cn]):
                            _ndup += 1
                            continue
                        _new_det[_cn].append(
                            {'x': int(round(_px)), 'y': int(round(_py)),
                             'fitness': 1.0, 'px': 1, 'source': 'v45-jump',
                             'verdict': 'REAL'})
                        _nj += 1
                _new_det[_cn].sort(key=lambda _q3: _q3['x'])
                # step 5 has to see them too.  The stage runs after the loop
                # that built _s5, so without this the corrector optimises a set
                # with no jumps in it and its output -- which replaces the
                # detections wholesale -- silently drops every one of them.
                for _e5 in _s5:
                    if _e5['name'] != _cn:
                        continue
                    _e5['init_points'] = [
                        {'cx': float(_d5['x']), 'cy': float(_d5['y']),
                         'class_name': _cn, 'class_idx': _i, 'confidence': 1.0}
                        for _d5 in _new_det[_cn]]
                    _e5['init_suppressed'] = [
                        _q5 for _q5 in _e5['init_suppressed']
                        if not any((_q5['cx'] - _d5['x']) ** 2
                                   + (_q5['cy'] - _d5['y']) ** 2 <= PT_MERGE_TOL ** 2
                                   for _d5 in _new_det[_cn])]
            _kinds = {}
            for _q4 in _vres.values():
                for _r4 in _q4:
                    _kinds[_r4['kind']] = _kinds.get(_r4['kind'], 0) + 1
            print(f"  [v45] vertical strokes {len(_strokes)} -> {_kinds}; "
                  f"jump points {_njall} ({_ndup} already had a point, "
                  f"+{_nj} new)")
        except Exception as _ev:
            print(f"  [v45] vertical-jump stage skipped: {_ev}")

        if _new_det:
            all_detections = _new_det
            all_results = _new_res
            all_tcaps_out = {k: [] for k in _new_det}
            _V45_STEP5 = {'grid_xs': [float(g) for g in _grid],
                          'plot_area': list(_pa), 'legend_box': list(_lb),
                          'curves': _s5}
            with open(os.path.join(OUT_DIR, 'step5_inputs.json'), 'w') as _f5:
                json.dump(_V45_STEP5, _f5, indent=1)
            print(f"  [v45] {sum(len(v) for v in _new_det.values())} points "
                  f"in {time.time() - _t0:.1f}s -> step5_inputs.json")

            if STEP5_DIR:
                try:
                    _run_step5(img, _masks, _fur, _V45_STEP5, STEP5_DIR,
                               OUT_DIR, STEP5_ITERS, print)
                    _res5 = json.load(open(os.path.join(OUT_DIR,
                                                        'step5_result.json')))
                    for _r in _res5:
                        if _r['points']:
                            all_detections[_r['name']] = [
                                {'x': int(round(px)), 'y': int(round(py)),
                                 'fitness': 1.0, 'px': 1, 'source': 'step5'}
                                for px, py in sorted(_r['points'])]
                    print(f"  [v45] step 5 applied to {len(_res5)} curve(s)")
                except Exception as _e5:
                    print(f"  [v45] step 5 skipped: {_e5}")
        else:
            print("  [v45] no curves matched -- keeping v44 detections")
    except Exception as _e:
        import traceback; traceback.print_exc()
        print(f"  [v45] pipeline failed ({_e}) -- keeping v44 detections")
...
# Default OFF, to match unified_server.py, which sets NO_OUTLIER_FILTER=1 on
# every colour job because step 5 replaces these filters.  Running the CLI with
# them ON gave a different answer from the GUI for the same image, and the
# difference was not harmless: the geometric filter drops a point that deviates
# from a local line fit through its neighbours, which is exactly what the two
# ends of a dosing jump do -- it removed the jumps and nothing else.
_OUTLIER_ON = bool(_args.outlier_filter) and not os.environ.get('NO_OUTLIER_FILTER')
print('  [outlier filter] ' + ('ON (--outlier-filter)' if _OUTLIER_ON
                               else 'OFF (default; step 5 replaces it)'))
if _OUTLIER_ON:
    _tot_drop = 0
